...
@pytest.mark.asyncio
async def test_token_rotation_and_revocation(async_client: AsyncClient, redis_client: Redis):
    ...
    # 2. Use refresh token
    res2 = await async_client.post("/v1/auth/refresh", json={"refresh_token": refresh_token})
    assert res2.status_code == 200
    new_refresh_token = res2.json()["refresh_token"]

    # 3. Use old refresh token AGAIN - should fail (rotation protection)
    res3 = await async_client.post("/v1/auth/refresh", json={"refresh_token": refresh_token})
    ...
    res5 = await async_client.post("/v1/auth/refresh", json={"refresh_token": new_refresh_token})
...
async def test_require_admin_role_enforcement(async_client: AsyncClient):
    ...
    res = await async_client.post("/v1/models/promote", json={"challenger_id": "123", "reason": "test"})
